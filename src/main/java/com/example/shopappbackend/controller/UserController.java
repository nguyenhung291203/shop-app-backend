...
public class UserController {
    ...
    public ResponseEntity<?> getAllUsers() {
        return ResponseEntity.ok(ResponseApi.builder().data(userService.getAllUsers()).message("Get success").build());
    }
    ...
    public ResponseEntity<?> getUserDetail(@RequestHeader("Authorization") String jwtToken) {
        ...
        return ResponseEntity.ok(ResponseApi.builder().data(userService.getUserDetail(token)).build());
    }

    @PostMapping("/register")
    public ResponseEntity<?> register(@Valid @RequestBody UserRegisterDTO userDTO) {
        log.info("Register user: {}", userDTO);
        return ResponseEntity.status(HttpStatus.CREATED).body(ResponseApi.builder().message(localizationUtil.getLocaleResolver(MessageKey.REGISTER_SUCCESS)).data(userService.register(userDTO)).build());
    }

    @PostMapping("/login")
    public ResponseEntity<?> login(@Valid @RequestBody UserLoginDTO userLoginDTO) {
        return ResponseEntity.ok(ResponseApi.builder().data(userService.login(userLoginDTO)).message(localizationUtil.getLocaleResolver(MessageKey.LOGIN_SUCCESSFULLY)).build());
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> updateUser(@PathVariable @Valid Long id, @Valid @RequestBody UserUpdateDTO userUpdateDTO) {
        return ResponseEntity.ok(ResponseApi.builder().data(userService.updateUser(id, userUpdateDTO)).message("Cập nhật thành công").build());
    }
}
